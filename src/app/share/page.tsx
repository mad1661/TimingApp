...
export default function SharePage() {
  ...
  const refresh = useCallback(async (silent = false) => {
    ...
    setError(null);
    try {
      // Bust any browser/proxy caching with a unique query param + no-store
      // and pull a fresh scrape every time.
      const res = await fetch(`/api/public-fetch?_=${Date.now()}`, {
        method: "POST",
        cache: "no-store",
      ...
      // Replace whole object reference so React re-renders even when the new
      // payload is structurally identical to the previous one.
      setData({ ...json });
