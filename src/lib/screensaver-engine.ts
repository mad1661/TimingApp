/**
 * Drag Strip Valley — a Silicon Valley-intro-style animated screensaver engine.
 *
 ...
 * a scoreboard.
 *
 * This module is framework-free on purpose: it drives the in-app
 * /screensaver page AND is bundled standalone (via scripts/build-saver-html.ts)
 * into the native macOS screensaver in macos-screensaver/. Keep React and
 * Next.js imports out of it.
 */
...
/**
 * Starts the animation on the given canvas (sized to fill its CSS box,
 * HiDPI-aware). Returns a cleanup function that stops the loop.
 */
export function startScreensaver(canvas: HTMLCanvasElement): () => void {
