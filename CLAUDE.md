...
- **Scoring days** — `scoringDates` in the event config lists the days (`YYYY-MM-DD`) whose runs count; empty means all of them. A track often runs more than one event under a single event code, so only some days are the E.T. Finals. Filtering happens *before* anything else in `computeEtFinalsStandings`, so categories, rounds and standings all reflect the chosen days; the `days` array it returns catalogues every day in the data (from the unfiltered runs) with its run count and classes, so the page can offer days that are currently switched off. Runs with no parseable date are never dropped.
